...
dat_path = '/dlib_model/shape_predictor_68_face_landmarks.dat'
if not pwd:
    dat_path = '.' + dat_path
lmark_predictor = dlib.shape_predictor(pwd + dat_path)
...
def run(input_dir):
    ...
    for f_name in f_list:
        ...
        opath = 'results/' + str(f_name) + '.txt'
        with open(opath, 'w') as ofile:
            ofile.write(str(prob))
        print('Prob: ' + str(prob))
    sess.close()
    return prob_list
